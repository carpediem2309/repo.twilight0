...
def BuildMenu():
    link = OPEN_URL('https://raw.githubusercontent.com/Twilight0/repo.twilight0/master/plugin.program.Twilight0/builds').replace('\n','').replace('\r','')
...
def AddonsMenu():
    link = OPEN_URL('https://raw.githubusercontent.com/Twilight0/repo.twilight0/master/plugin.program.Twilight0/addons').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,fanart,description in match:
...
def MAINTENANCE():
    ...
    addDir('DELETE PACKAGES','url',7,ART+'purge-packages.png',FANART,'')
