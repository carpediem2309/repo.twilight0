"""
This software is provided as is under the terms of GPL v3 Licence
...
addon_id = 'plugin.program.Twilight0'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID = 'plugin.program.Twilight0'
AddonTitle = "Twilight0 Wizard"
...
PATH = "Twilight0 Wizard Toolbox"
BASEURL = "https://github.com/Twilight0/repo.twilight0/"
H = 'http://'
...
def INDEX():
    ...
    addDir('TOOLS', BASEURL, 3, ART + 'tools.png', FANART, '')
...
def BuildMenu():
    link = OPEN_URL(
        'https://raw.githubusercontent.com/Twilight0/repo.twilight0/master/plugin.program.Twilight0/builds').replace('\n', '').replace('\r', '')
...
def AddonsMenu():
    link = OPEN_URL(
        'https://raw.githubusercontent.com/Twilight0/repo.twilight0/master/plugin.program.Twilight0/addons').replace('\n', '').replace('\r', '')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name, url, iconimage, fanart, description in match:
...
def TOOLS():
    ...
    addDir('PURGE PACKAGES', 'url', 7, ART + 'purge-packages.png', FANART, '')
...
def disclaimer():
    TextBoxes('Disclaimer',
              'This software is provided as is, under the terms of GPL v3 Licence. Use at your own risk')
...
def wizard(name, url, description):
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("DOWNLOAD COMPLETE", 'To ensure all changes are saved you must now close Kodi',
              'to force close Kodi. Click ok,', 'DO NOT use the quit/exit options in Kodi.')
    killxbmc()
...
def addons(name, url, description):
    path = xbmc.translatePath(os.path.join('special://home/addons', 'packages'))
    dp = xbmcgui.DialogProgress()
    dp.create("Twilight0 Wizard ", "Downloading ", '', 'Please Wait')
    lib = os.path.join(path, name + '.zip')
    ...
    try:
        for root, dirs, files in os.walk(packages_cache_path):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Delete Package Cache Files", str(file_count) + " files found",
                    ...
                    dialog = xbmcgui.Dialog()
                    dialog.ok("Twilight0 Wizard", "Packages have now been purged", "Have Fun")
    except:
        dialog = xbmcgui.Dialog()
...
def deletecachefiles(url):
    ...
    if os.path.exists(wtf_cache_path) == True:
        for root, dirs, files in os.walk(wtf_cache_path):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Delete WTF Cache Files", str(file_count) + " files found",
                                "Do you want to delete them?"):
    ...
    if os.path.exists(channel4_cache_path) == True:
        for root, dirs, files in os.walk(channel4_cache_path):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Delete 4oD Cache Files", str(file_count) + " files found",
                                "Do you want to delete them?"):
    ...
    iplayer_cache_path = os.path.join(
        xbmc.translatePath('special://profile/addon_data/plugin.video.iplayer/iplayer_http_cache'), '')
    if os.path.exists(iplayer_cache_path) == True:
        for root, dirs, files in os.walk(iplayer_cache_path):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Delete BBC iPlayer Cache Files", str(file_count) + " files found",
                                "Do you want to delete them?"):
    ...
    downloader_cache_path = os.path.join(
        xbmc.translatePath('special://profile/addon_data/script.module.simple.downloader'), '')
    if os.path.exists(downloader_cache_path) == True:
        for root, dirs, files in os.walk(downloader_cache_path):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Delete Simple Downloader Cache Files", str(file_count) + " files found",
                                "Do you want to delete them?"):
    ...
    if os.path.exists(itv_cache_path) == True:
        for root, dirs, files in os.walk(itv_cache_path):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Delete ITV Cache Files", str(file_count) + " files found",
                                "Do you want to delete them?"):
    ...
    if os.path.exists(temp_cache_path) == True:
        for root, dirs, files in os.walk(temp_cache_path):
            ...
            if file_count > 0:
                ...
                dialog = xbmcgui.Dialog()
                if dialog.yesno("Delete TEMP dir Cache Files", str(file_count) + " files found",
                                "Do you want to delete them?"):
...
def killxbmc():
    choice = xbmcgui.Dialog().yesno('Force Close Kodi', 'You are about to close Kodi', 'Would you like to continue?',
...
def FRESHSTART(params):
    ...
    yes_pressed = plugintools.message_yes_no(AddonTitle, "Do you wish to remove everything but", "Twilight0 Wizard")
    if yes_pressed:
        ...
        try:
            ...
            if not failed:
                ...
                plugintools.message(AddonTitle, "The process is complete, you're now back to a fresh Kodi configuration with Twilight0 Wizard", "Please reboot your system or restart Kodi in order for the changes to be applied.")
            else:
                plugintools.log("freshstart.main_list User files partially removed")
                plugintools.message(AddonTitle, "The process is complete, you're now back to a fresh Kodi configuration with Twilight0 Wizard to install a build", "Please reboot your system or restart Kodi in order for the changes to be applied.")
        except:
            plugintools.message(AddonTitle, "Problem found", "Your settings has not been changed")
            import traceback
            plugintools.log(traceback.format_exc()); plugintools.log("freshstart.main_list NOT removed")
            plugintools.add_item(action="", title="Now Exit Kodi", folder=False)
    else:
        plugintools.message(AddonTitle, "Your settings", "has not been changed")
