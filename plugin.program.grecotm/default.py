#Very special thanks to Andrewrx for the original wizard code 
#### All Code was tweaked by Andrewrx #####
...
AddonID = 'plugin.program.grecotm'
...
def MainMenu():
	...
	if FireTVBuildMenu == 'true':
		addFolder('folder','GRecoTM Builds','none', 'FireTVBuildMenu', 'wizardicon.png','','','')
	if Maintenance == 'true':
		addFolder('folder','Maintenance and Tools','fanart', 'Tools', 'maintenance.png','','','')	
	setView('movies', 'MAIN')
def FireTVBuildMenu():
    link = OPEN_URL('http://4greeks.comlu.com/Wizard/wizardGR.xml').replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp.create("[COLOR dodgerblue][B]GRecoTM Wizard[/COLOR][/B]","Downloading ",'', 'Please be Patient')
    ...
    Dialog = xbmcgui.Dialog()
    Dialog.ok("[COLOR dodgerblue][B]GRecoTM Wizard[/COLOR][/B]", 'Unfortunately the only way to get the new changes to stick is', 'to force close kodi. Click ok to force Kodi to close,', 'DO NOT use the quit/exit options in Kodi.')
    killkodi()
...
elif mode == 'Wipe_Kodi' : GoDev.freshstart() #Wipe_Kodi
