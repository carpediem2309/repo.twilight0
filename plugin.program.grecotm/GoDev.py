...
ADDON          =  xbmcaddon.Addon(id='plugin.program.grecotm')
...
def freshstart():
    ...
    choice = xbmcgui.Dialog().yesno('[COLOR dodgerblue][B]GRecoTM Wizard[/COLOR][/B]  [COLOR=red][B]WARNING !!![/COLOR][/B]', '          This will erase all data and reset Kodi to defaults!','   Kodi will try to force close after the process is complete.', '                             [COLOR=red][B]Do you want to continue?[/COLOR][/B]', nolabel='Nope',yeslabel='YES')
    if choice == 0:
        return
    elif choice == 1:
        pass
	...
	dp.create("GRecoTM Wizard","Please Wait")
	dp.update(1)
	...
	for root, dirs, files in os.walk(homedir):
			for f in files:
							try:
									if f == "kodi.log" or f == "build.zip":
										...
										if 'plugin.video.grecotm' not in path and 'plugin.program.grecotm' not in path and 'script.module.elementtree' not in path:
			...
			for d in dirs:
							try:
									if d == "Database" or d == "addon_data" or d == "userdata" or d == "packages" or d == "addons" or d == "Thumbnails" or d == "temp" or d == "plugin.video.Bugatsinho" or d == "plugin.program.Bugatsinho" or d == "script.module.elementtree":
										pass
									else:
	...
	for root, dirs, files in os.walk(addondatafolder):
			for f in files:
							try:
									...
									path = os.path.join(root, f)
									if 'plugin.video.grecotm' not in path:
	...
	for root, dirs, files in os.walk(mainaddondir):
			...
			for d in dirs:
							try:
									if d == "packages" or d == "plugin.video.grecotm" or d == "plugin.program.grecotm" or d == "script.module.elementtree":
										pass
									else:
										if 'plugin.video.grecotm' not in path and 'plugin.program.grecotm' not in path and 'script.module.elementtree' not in path:
...
def killkodi():
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("[COLOR dodgerblue][B]GRecoTM Wizard[/COLOR][/B]", "[COLOR red][B]Kodi needs to be closed. Press OK to attempt to close Kodi[/COLOR][/B]")
    myplatform = platform()
...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dialogprocess.create("GRecoTM","Archiving...",'', 'Please Wait')
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.grecotm' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def Destroy_Path(path):
    dialogprocess.create("GRecoTM","Cleaning...",'', 'Please Wait')
...
def Check_Path():
    if zip=='':
     if dialog.yesno("GRecoTM Wizard", "You Have Not Set Your Storage Path", 'Set The Storage Path Now ?',''):
        ADDON.openSettings()
        print '######### ZIP DIRECTORY #########'
...
def RestoreIt():
    ...
    dialog = xbmcgui.Dialog()
    if zip == '':
        dialog.ok('GRecoTM Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    dialogprocess.create("GRecoTM Wizard","Restoring",'', 'Please Wait')   
    lib=xbmc.translatePath(os.path.join(zip,'backup.zip'))
    Read_Zip(lib)
    dialogprocess.create("GRecoTM Wizard","Checking ",'', 'Please Wait')
    ...
    dialogprocess.close()
    dialog.ok("GRecoTM Wizard", "All Done, DONT PRESS OK", "Wait a 5 minutes and pull the Power Cord then","")	
def Backupzip():  
    if zip == '':
        dialog.ok('GRecoTM Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    ...
    dialogprocess.create("GRecoTM Wizard","Backing Up",'', 'Please Wait')
    ...
    zipobj.close()
    dialogprocess.close()
    dialog.ok("GRecoTM Wizard", "You Are Now Backed Up", '','')
